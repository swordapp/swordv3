...
on the number or type of packaging formats that the client/server support, though see the section {% link Packaging Formats %} for the
packages that MUST be supported by the server.


## Announcing Support for Packaged Content Deposit

The {% link Service Document %} uses the `acceptPackaging` field to indicate that a Service will accept deposits of a particular 
...
## Package description in Status documents
...
```json
{% json_extract
    source=examples/packaged-content_status_types.json,
